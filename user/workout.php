...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'complete') {
    $ins = $db->prepare("
        INSERT INTO user_workout_progress (user_id, day_id, exercise_id, completed, completed_at, day_number)
        VALUES (?, ?, 1, 1, NOW(), ?)
        ON DUPLICATE KEY UPDATE completed=1, completed_at=NOW()
    ");
    $ins->execute([$user_id, $day, $day]);
    header('Location: /myfitcal_system/user/complete.php?day='.$day); exit;
}

// Which exercise just got completed via camera (0-based index from redirect)
$just_completed = isset($_GET['completed']) ? (int)$_GET['completed'] : -1;

$uq = $db->prepare("SELECT name FROM users WHERE id=?");
$uq->execute([$user_id]);
$user = $uq->fetch();

// Load already-completed exercises for this day from DB
$cq = $db->prepare("SELECT exercise_id FROM user_workout_progress WHERE user_id=? AND day_number=? AND completed=1");
$cq->execute([$user_id, $day]);
$completedIds = $cq->fetchAll(\PDO::FETCH_COLUMN);

$exercise_poses = [
    'Jumping Jacks'=>'jump','Wall Push-Up'=>'pushup','Chair Squat'=>'squat',
    'Standing March'=>'march','Glute Bridge'=>'bridge','Seated Leg Raise'=>'legrise',
    'Knee Push-Up'=>'pushup','Standing Side Bend'=>'bend','Push-Up'=>'pushup',
    'Bodyweight Squat'=>'squat','Plank'=>'plank','Reverse Lunge'=>'squat',
    'Tricep Dip'=>'pushup','Superman'=>'bridge','High Knees'=>'march',
    'Mountain Climber'=>'march','Diamond Push-Up'=>'pushup','Jump Squat'=>'jump',
    'Burpee'=>'jump','Pike Push-Up'=>'pushup','Bulgarian Split Squat'=>'squat',
    'Plank to Push-Up'=>'plank','Speed Skater'=>'jump','V-Up'=>'legrise',
    'Clap Push-Up'=>'pushup','Pistol Squat'=>'squat','Burpee Tuck Jump'=>'jump',
    'Archer Push-Up'=>'pushup','Single Leg Deadlift'=>'bend','Dragon Flag'=>'legrise',
    'Lateral Bound'=>'jump','L-Sit Hold'=>'plank',
];
...
$exercise_videos = [
    'Jumping Jacks'=>'/myfitcal_system/video/jumping jack.mp4','Wall Push-Up'=>'/myfitcal_system/video/wall push up.mp4',
    'Chair Squat'=>'/myfitcal_system/video/squats.mp4','Standing March'=>'/myfitcal_system/video/standing march.mp4',
    'Glute Bridge'=>'/myfitcal_system/video/glute bridge.mp4','Seated Leg Raise'=>'/myfitcal_system/video/seated leg raise.mp4',
    'Knee Push-Up'=>'/myfitcal_system/video/knee push up.mp4','Standing Side Bend'=>'/myfitcal_system/video/standing side bend.mp4',
    'Push-Up'=>'/myfitcal_system/video/push up.mp4','Bodyweight Squat'=>'/myfitcal_system/video/bodyweight.mp4',
    'Plank'=>'/myfitcal_system/video/plank.mp4','Reverse Lunge'=>'/myfitcal_system/video/lunges.mp4',
    'Tricep Dip'=>'/myfitcal_system/video/tricep dips.mp4','Superman'=>'/myfitcal_system/video/superman.mp4',
    'High Knees'=>'/myfitcal_system/video/high knee.mp4','Mountain Climber'=>'/myfitcal_system/video/mountain.mp4',
    'Diamond Push-Up'=>'/myfitcal_system/video/diamond push up.mp4','Jump Squat'=>'/myfitcal_system/video/jumping squat.mp4',
    'Burpee'=>'/myfitcal_system/video/burpees.mp4','Pike Push-Up'=>'/myfitcal_system/video/pike push up.mp4',
    'Bulgarian Split Squat'=>'/myfitcal_system/video/Bulgarian split squat.mp4','Plank to Push-Up'=>'/myfitcal_system/video/plank push up.mp4',
    'Speed Skater'=>'/myfitcal_system/video/speed skater.mp4','V-Up'=>'/myfitcal_system/video/v up.mp4',
    'Clap Push-Up'=>'/myfitcal_system/video/clap push up.mp4','Pistol Squat'=>'/myfitcal_system/video/pistol squat.mp4',
    'Burpee Tuck Jump'=>'/myfitcal_system/video/burpees tack jump.mp4','Archer Push-Up'=>'/myfitcal_system/video/Archer push up.mp4',
    'Single Leg Deadlift'=>'/myfitcal_system/video/Single leg deadlift.mp4','Dragon Flag'=>'/myfitcal_system/video/dragon flag.mp4',
    'Lateral Bound'=>'/myfitcal_system/video/Lateral bound.mp4','L-Sit Hold'=>'/myfitcal_system/video/L sit hold.mp4',
];
?>
<!DOCTYPE html>
<html lang="en">
...
.sb-foot{padding:10px 14px;border-top:1px solid rgba(255,255,255,.06);display:flex;align-items:center;gap:9px;flex-shrink:0;margin-top:auto;}
...
.ex-media{position:relative;width:100%;height:320px;overflow:hidden;background:#000;}
.ex-video{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;display:block;}
...
/* Action buttons row */
.action-row{display:flex;gap:8px;margin-bottom:0;}
...
.btn-camera:hover{background:#1d4ed8;color:#fff;}
.btn-camera.disabled{opacity:.4;pointer-events:none;}
...
.btn-finish{display:block;background:#1c1917;color:#fafaf9;border:none;border-radius:6px;padding:12px 2rem;font-family:'DM Sans',sans-serif;font-weight:600;font-size:13px;cursor:pointer;width:100%;text-decoration:none;transition:background .15s;}
.btn-finish:hover{background:#292524;color:#fafaf9;}

/* Toast notification for completed exercise */
.toast{
...
<div class="main">
  <div class="topbar">
    ...
    <div class="topbar-r">
      <a href="/myfitcal_system/user/<?= $is_female ? 'dashboard_female' : 'dashboard' ?>.php" class="tb-btn"></a>
    </div>
  </div>
  ...
  <div class="content">
    ...
    <?php foreach ($today['exercises'] as $i => $ex):
      ...
      $ex_id      = $i + 1;
      // Parse rep number from string like "15 reps" or just "15"
      $reps_raw   = $ex['reps'];
      preg_match('/\d+/', $reps_raw, $rm);
      $reps_num   = $rm[0] ?? 10;
      $sets_num   = (int)$ex['sets'];
      // Check if already done from DB
      $is_done_db = in_array($ex_id, $completedIds);
    ?>
    <div class="ex-card <?= ($i===0 && $just_completed===-1) ? 'active' : ($is_done_db ? 'done-card' : '') ?>"
         id="card<?= $i ?>">
      <div class="ex-card-header" onclick="toggleCard(<?= $i ?>)">
        <div class="ex-num-badge" id="badge<?= $i ?>">
          <?php if ($is_done_db): ?><i class="bi bi-check-lg"></i><?php else: ?><?= $i+1 ?><?php endif; ?>
        </div>
        ...
        <div class="ex-status" id="status<?= $i ?>"><?= $is_done_db ? '✅' : '⬜' ?></div>
      </div>
      <div class="ex-body">
        <div class="ex-media">
          <?php if ($video_src): ?>
        ...
        <div class="ex-details">
          ...
          <div class="action-row">
            ...
            <a href="/myfitcal_system/user/workout-camera.php?exercise=<?= urlencode($ex['name']) ?>&sets=<?= $sets_num ?>&reps=<?= $reps_num ?>&day=<?= $day ?>&ex_index=<?= $i ?>&exercise_id=<?= $ex_id ?>"
               class="btn-camera <?= $is_done_db ? 'disabled' : '' ?>"
               id="camBtn<?= $i ?>">
              <i class="bi bi-camera-video-fill"></i> Start Camera
            </a>
            <!-- Manual mark done button -->
            <button class="btn-done <?= $is_done_db ? 'completed' : '' ?>"
                    id="btn<?= $i ?>"
                    onclick="markDone(<?= $i ?>)"
                    <?= $is_done_db ? 'disabled' : '' ?>>
              <?php if ($is_done_db): ?>
...
<div class="toast" id="toast">
  ...
  <span id="toastMsg">Exercise completed!</span>
</div>

<script>
var total       = <?= count($today['exercises']) ?>;
var done        = new Array(total).fill(false);
var justCompleted = <?= $just_completed ?>;  // -1 if none

// Pre-mark exercises already done in DB
<?php foreach ($today['exercises'] as $i => $ex):
  $ex_id = $i + 1;
  if (in_array($ex_id, $completedIds)): ?>
done[<?= $i ?>] = true;
<?php endif; endforeach; ?>

// On page load — handle redirect back from camera
window.addEventListener('DOMContentLoaded', function() {
  updateProgress();

  if (justCompleted >= 0) {
    // Auto mark the completed exercise as done in UI
    applyDone(justCompleted);

    // Show toast
    var exName = document.getElementById('card' + justCompleted)
                         .querySelector('.ex-card-name').textContent;
    showToast('✓ ' + exName + ' completed!');

    // Auto-open next exercise
    var next = justCompleted + 1;
    if (next < total && !done[next]) {
      setTimeout(function() {
        document.getElementById('card' + next).classList.add('active');
    ...
    if (done.every(Boolean)) {
      setTimeout(function() {
        document.getElementById('finishSection').classList.add('show');
        document.getElementById('finishSection').scrollIntoView({ behavior:'smooth' });
      }, 800);
    }
  }
});

function applyDone(i) {
  if (done[i]) return;
  done[i] = true;
  ...
  var camBtn = document.getElementById('camBtn' + i);
